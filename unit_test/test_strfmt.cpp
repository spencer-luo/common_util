...
TEST(StrFmtTest, FmtTimeZone)
{
    ...
    EXPECT_EQ(cutl::fmt_timezone_offset(8, true), "UTC+08:00");

    // show_minutes=false 只展示小时
    EXPECT_EQ(cutl::fmt_timezone_offset(0, false), "UTC+00");
    EXPECT_EQ(cutl::fmt_timezone_offset(8, false), "UTC+08");
    EXPECT_EQ(cutl::fmt_timezone_offset(-5, false), "UTC-05");
}

TEST(StrFmtTest, FmtTimeZoneMinutes)
{
    // 默认 show_minutes=true，始终带分钟（整点也打出 :00）
    ...
    EXPECT_EQ(cutl::fmt_timezone_offset_min(480, true), "UTC+08:00");

    // show_minutes=false 只展示小时，分钟部分被丢弃（印度 330 → UTC+05）
    EXPECT_EQ(cutl::fmt_timezone_offset_min(0, false), "UTC+00");
    EXPECT_EQ(cutl::fmt_timezone_offset_min(480, false), "UTC+08");
    EXPECT_EQ(cutl::fmt_timezone_offset_min(330, false), "UTC+05");
    EXPECT_EQ(cutl::fmt_timezone_offset_min(345, false), "UTC+05");
    EXPECT_EQ(cutl::fmt_timezone_offset_min(-210, false), "UTC-03");
    EXPECT_EQ(cutl::fmt_timezone_offset_min(840, false), "UTC+14");
}

TEST(StrFmtTest, FmtSystemTimezone)
{
    const int offset_min = cutl::get_timezone_offset_min();

    // 默认带分钟，与 fmt_timezone_offset_min(..., true) 一致
    auto with_min = cutl::fmt_system_timezone();
    EXPECT_EQ(with_min, cutl::fmt_timezone_offset_min(offset_min, true));
    EXPECT_EQ(with_min.substr(0, 3), "UTC");
    EXPECT_NE(with_min.find(':'), std::string::npos);

    // show_minutes=false 只展示小时，不应出现冒号
    auto hour_only = cutl::fmt_system_timezone(false);
    EXPECT_EQ(hour_only, cutl::fmt_timezone_offset_min(offset_min, false));
    EXPECT_EQ(hour_only.substr(0, 3), "UTC");
    EXPECT_EQ(hour_only.find(':'), std::string::npos);
}
