...
{
    PrintSubTitle("cutl::dynamic_bitmap");

    cutl::dynamic_bitmap dynamicBitmap;
...
void test_roaring_bitmap()
{
    ...
    cutl::roaring_bitmap andBitmap = bitmap1 & bitmap2;
    std::cout << "andBitmap size: " << andBitmap.size() << ", count: " << andBitmap.count()
              << std::endl;
    cutl::roaring_bitmap orBitmap = bitmap1 | bitmap2;
